...
adminRoute.get('/viewcourse', async(req,res)=>{
    try{
        ...
        res.send(Array.from(course.entries())) //built in function used to create array for course.array of array
    }
else{
...
adminRoute.get('/getcoursequery',(req,res)=>
{
    ...
    console.log(req.query.CourseName) // CourseName=Computer from url.it print in console
            ...
            if(addcourse.has(CourseName))
                {
                    ...
                    addcourse.set(CourseName,{CourseName,NewCourseType});
                    console.log(addcourse.get(CourseName));
                    res.status(200).json({Messagee:"Put Course Successfully"});
                }
                else
